...
% %  14/09/2023
% function preproEEG(pathIn,nameOutpp,perceived)
%     rng default %TO MAKERESULTS EASIER TO RESPRODUCE
%     idNum = pathIn(end-7:end-4); 
%     [~,~,~] = eeglab;
% % (1) Cargar datos .edf
%     EEG = pop_biosig(pathIn);
% % (2) Downsample
...
% % (6.1) Rename Epochs to perceived
%  if (perceived) 
%     id_str = idNum %sprintf('ID%02d', i);  
%     Events = {'33028' , '33029' , '33030' , '33031',...
%             '33032' , '33033'  ,'33034',  '33035',...
%             '33024' , '33025' , '33026' , '33027', '19999'};
%     dataPath = 'D:\shared_git\MaestriaThesis\data';
%     file_path = fullfile(dataPath,id_str,['R_', id_str, '.txt']);
% 
%     % Read the entire file as a string
%     file_contents = fileread(file_path);
%     filtered_string = str2num(file_contents');
...
%     %EEG = pop_interp(EEG, chanlocs_original, 'spherical');  
% % (7) Decomposing constant fixed-source noise/artifacts/signals (ICA)
%  %TO MAKERESULTS EASIER TO RESPRODUCE according to makoto tips
%     EEG = pop_runica(EEG, 'icatype', 'runica', 'extended', 1, 'lrate', 1e-5, 'maxsteps', 2000,'interrupt','off'); % 1300th iterations to converge.
% % (8) Remove ICs artifacts(ICA)
%     EEG = iclabel(EEG);
%     EEG = pop_icflag(EEG, [NaN NaN;0.6 1;0.6 1;0.6 1;0.6 1;0.6 1;0.6 1]);
%     EEG = pop_subcomp(EEG,find(EEG.reject.gcompreject), 0,0);
