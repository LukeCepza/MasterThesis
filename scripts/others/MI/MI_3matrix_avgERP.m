        ...
        if ismember(channels(idx_1),{EEG1.chanlocs.labels})
            ...
            for idx_2 = 1:22
                if ismember(channels(idx_2),{EEG1.chanlocs.labels})
                    d2 = 1+d2;
                    MIM_triple(1,idx_1,idx_2) = mutual_information(A(d1,:),B(d2,:), 'numBins',13,'freedmanDiaconisRule', false);   
                end
            end
        end
    end
        ...
        if ismember(channels(idx_1),{EEG2.chanlocs.labels})
            ...
            for idx_2 = 1:22
                if ismember(channels(idx_2),{EEG2.chanlocs.labels})
                    d2 = 1+d2;
                    MIM_triple(2,idx_1,idx_2) = mutual_information(B(d1,:),C(d2,:), 'numBins',13,'freedmanDiaconisRule', false);   
                end
            end
        end
    end
        ...
        if ismember(channels(idx_1),{EEG3.chanlocs.labels})
            ...
            for idx_2 = 1:22
                if ismember(channels(idx_2),{EEG3.chanlocs.labels})
                    d2 = 1+d2;
                    MIM_triple(3,idx_1,idx_2) = mutual_information(C(d1,:),A(d2,:),'numBins',13, 'freedmanDiaconisRule', false);   
                end
            end
        end
    end
